...
const DATABASE_TYPE = (process.env.DB_TYPE || 'postgres').toLowerCase(); // 'mysql' | 'postgres' | 'sqlite'

let sequelize: Sequelize;

// Lightweight, DB-less path for local/unit testing
// Enable with USE_SQLITE_FOR_TESTS=true (only affects NODE_ENV==='test') or DB_TYPE==='sqlite'
if (DATABASE_TYPE === 'sqlite' || (process.env.NODE_ENV === 'test' && process.env.USE_SQLITE_FOR_TESTS === 'true')) {
